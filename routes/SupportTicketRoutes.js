...
router.post('/:tenantId/tickets', (req,res,next) => {
  console.log(req.body)
  next()}, authenticateTenantOwner, validateSupportTicket, createTicket);

// Get all tickets for a tenant
router.get('/:tenantId/tickets', authenticateTenantOwner, authorizePermission(['admin', 'support']), getAllTickets);

// Get a specific ticket for a tenant
router.get('/:tenantId/tickets/:id', authenticateTenantOwner, getTicketById);

// Update a specific ticket for a tenant
router.put('/:tenantId/tickets/:id', authenticateTenantOwner, validateSupportTicket, updateTicket);
